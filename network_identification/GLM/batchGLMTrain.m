function [optpara, cost, inputs] = batchGLMTrain(trainX, minlag, maxlag,lambda,reg)
...
for i=1:batchN
    disp(i);
    inputs{i,1}=GLMPrepareData(trainX{i,1},minlag,maxlag);
...
[optpara, cost] = minFunc( @(p) batchGLMGrad(p,inputs,minlag,maxlag,lambda,reg), para, options);
